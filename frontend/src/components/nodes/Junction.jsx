...
const Junction = ({ id, data, selected, type }) => {
  ...
  const leftPortCount = nodeState
    ? parseInt(nodeState.parameters.leftPorts, 10) || 0
    : 2;
  const rightPortCount = nodeState
    ? parseInt(nodeState.parameters.rightPorts, 10) || 0
    : 1;

  // Generate automatic port IDs based on the port counts
  const leftPorts = Array.from(
    { length: leftPortCount },
    (_, index) => `${index}`
  );
  ...
  useLayoutEffect(() => {
    ...
    const createNewEdge = (edge, portId) => ({
      ...edge,
      sourceHandle:
        edge.source === id ? `${id}-port-${portId}` : edge.sourceHandle,
      targetHandle:
        edge.target === id ? `${id}-port-${portId}` : edge.targetHandle,
    });
    ...
    if (needsUpdate) {
      console.log(`Updating edges for Junction node ${id}`);
      setEdges(newEdges);
    }
